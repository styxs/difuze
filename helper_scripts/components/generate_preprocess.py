...
class GeneratePreprocessed(Component):
    ...
    def perform(self):
        return _generate_llvm_preprocessed(self.kernel_src_dir, self.llvm_bc_out, self.curr_makeout, self.gcc_name, self.arch_num, self.clang_bin,
                                           command_output_dir=self.separate_out)

    def get_name(self):
        return "GeneratePreprocessed"
...
def _generate_llvm_preprocessed(kernel_src_dir, base_output_folder, makeout_file, gcc_prefix, target_arch, clang_path,
                                command_output_dir=None):
    output_llvm_sh_file = os.path.join(base_output_folder, 'llvm_generate_preprocessed.sh')
    ...
    os.chdir(curr_dir)
    log_info("Finished Building LLVM Preprocessed files")
    for i in range(len(return_vals)):
        if return_vals[i]:
            log_error("[-] Command Failed:" + llvm_cmds[i] + "\n")

    log_success("[+] Script containing all LLVM Preprocess Commands:" + output_llvm_sh_file)
    return True
